...
export default FinancialRatioContent;

interface FinancialRatioTableProps {
...
function FinancialRationTable({ financialRatios }: FinancialRatioTableProps) {
  const years = new Set<number>();
  const groupedItems: GroupedItems = {};

  // Process and group the data by subcategory
  financialRatios.forEach((item) => {
    years.add(item.year);
                        ...
                        <For each={sortedYears}>
                          {(year) => {
                            ...
                            if (
                              ...
                              return <Table.Cell>NI</Table.Cell>;
                            }
                            const numValue = parseFloat(value);
                            return (
                              <Table.Cell>
                                {isNaN(numValue) ? "NI" : numValue.toFixed(2)}
                              </Table.Cell>
                            );
                          }}
                        </For>
